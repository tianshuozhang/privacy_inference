...
Minionn::Minionn()
    ...
    filename = "Minionn.txt";
}

torch::Tensor Minionn::forward(torch::Tensor x) {
    x = act->forward(layer1->forward(x));
    x = pool->forward(x);
    x = act->forward(layer2->forward(x));
    x = pool->forward(x);
    x = x.view({ -1, 256 });
    x = act->forward(layer3->forward(x));
    x = act->forward(layer4->forward(x));
    return x;
}

ModuleInfo Minionn::GetModuleByName(const std::string& name) {
...
torch::Tensor Minionn::layer_forward(torch::Tensor x) {

    std::thread serverThread(std::bind(&Minionn::startServer, this, std::ref(x)));
    std::thread clientThread(std::bind(&Minionn::startClient, this, 5010));
    std::thread clientThread_1(std::bind(&Minionn::startClient, this, 5020));
    clientThread.join();
    clientThread_1.join();
    serverThread.join();
    WSACleanup();
...
void Minionn::startServer(torch::Tensor& data) {
...
void Minionn::startClient(int port) {
