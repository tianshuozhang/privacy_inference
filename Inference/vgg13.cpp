...
vgg13::vgg13()
    ...
    layer6(register_module("layer6", torch::nn::Conv2d(torch::nn::Conv2dOptions(64, 128, 3).stride(1).padding(1)))),
    layer8(register_module("layer8", torch::nn::Conv2d(torch::nn::Conv2dOptions(128, 128, 3).stride(1).padding(1)))),
    layer11(register_module("layer11", torch::nn::Conv2d(torch::nn::Conv2dOptions(128, 256, 3).stride(1).padding(1)))),
    layer13(register_module("layer13", torch::nn::Conv2d(torch::nn::Conv2dOptions(256, 256, 3).stride(1).padding(1)))),
    layer16(register_module("layer16", torch::nn::Conv2d(torch::nn::Conv2dOptions(256, 512, 3).stride(1).padding(1)))),
    layer18(register_module("layer18", torch::nn::Conv2d(torch::nn::Conv2dOptions(512, 512, 3).stride(1).padding(1)))),
    layer21(register_module("layer21", torch::nn::Conv2d(torch::nn::Conv2dOptions(512, 512, 3).stride(1).padding(1)))),
    layer23(register_module("layer23", torch::nn::Conv2d(torch::nn::Conv2dOptions(512, 512, 3).stride(1).padding(1)))),

    layer5(register_module("layer5", torch::nn::MaxPool2d(torch::nn::MaxPool2dOptions(2).stride(2).padding(0).dilation(1).ceil_mode(false)))),
    layer10(register_module("layer10", torch::nn::MaxPool2d(torch::nn::MaxPool2dOptions(2).stride(2).padding(0).dilation(1).ceil_mode(false)))),
    layer15(register_module("layer15", torch::nn::MaxPool2d(torch::nn::MaxPool2dOptions(2).stride(2).padding(0).dilation(1).ceil_mode(false)))),
    layer20(register_module("layer20", torch::nn::MaxPool2d(torch::nn::MaxPool2dOptions(2).stride(2).padding(0).dilation(1).ceil_mode(false)))),
    layer25(register_module("layer25", torch::nn::MaxPool2d(torch::nn::MaxPool2dOptions(2).stride(2).padding(0).dilation(1).ceil_mode(false)))),

    layer2(register_module("layer2", torch::nn::ReLU(torch::nn::ReLUOptions().inplace(true)))),
    layer4(register_module("layer4", torch::nn::ReLU(torch::nn::ReLUOptions().inplace(true)))),
    layer7(register_module("layer7", torch::nn::ReLU(torch::nn::ReLUOptions().inplace(true)))),
    layer9(register_module("layer9", torch::nn::ReLU(torch::nn::ReLUOptions().inplace(true)))),
    layer12(register_module("layer12", torch::nn::ReLU(torch::nn::ReLUOptions().inplace(true)))),
    layer14(register_module("layer14", torch::nn::ReLU(torch::nn::ReLUOptions().inplace(true)))),
    layer17(register_module("layer17", torch::nn::ReLU(torch::nn::ReLUOptions().inplace(true)))),
    layer19(register_module("layer19", torch::nn::ReLU(torch::nn::ReLUOptions().inplace(true)))),
    layer22(register_module("layer22", torch::nn::ReLU(torch::nn::ReLUOptions().inplace(true)))),
    layer24(register_module("layer24", torch::nn::ReLU(torch::nn::ReLUOptions().inplace(true)))),
    layer28(register_module("layer28", torch::nn::ReLU(torch::nn::ReLUOptions().inplace(true)))),
    layer31(register_module("layer31", torch::nn::ReLU(torch::nn::ReLUOptions().inplace(true)))),

    layer27(register_module("layer27", torch::nn::Linear(torch::nn::LinearOptions(512, 512).bias(true)))),
    layer30(register_module("layer30", torch::nn::Linear(torch::nn::LinearOptions(512, 512).bias(true)))),
    layer32(register_module("layer32", torch::nn::Linear(torch::nn::LinearOptions(512, 10).bias(true)))),

    layer26(register_module("layer26", torch::nn::Dropout(torch::nn::DropoutOptions(0.5).inplace(true)))),
    layer29(register_module("layer29", torch::nn::Dropout(torch::nn::DropoutOptions(0.5).inplace(true))))
...

{
    ...
    std::thread serverThread(std::bind(&vgg13::startServer, this));
    std::thread clientThread(std::bind(&vgg13::startClient, this, 5010));
    std::thread clientThread_1(std::bind(&vgg13::startClient, this, 5020));
    clientThread.detach();
...
ModuleInfo vgg13::GetModuleByName(const std::string& name) {
...
torch::Tensor vgg13::layer_forward(torch::Tensor x) {
...
void vgg13::startClient(int port)
{
...
torch::Tensor vgg13::forward(torch::Tensor x) {
